%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%------------- Lifting line model without induced velocities --------------
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
% Load liftcoeffiecients, Wagenigen data and geometry data
load LiftRadius.mat;
...
% Initialize matrices
beta=zeros(length(J),length(x));
phi=beta;
alpha=beta;
C_L=beta;
Vinf=beta;

% Goes through every advance number
for i =1:length(J)
    beta(i,:) = atan( J(i)./(pi*x) );
    phi(i,:) = atan(PdivD/pi./x);
...
% Calculate relative error between lifting line model and Wag. B
K_Ter = abs((K_Twag-K_T)./K_Twag);
K_Qer = abs((K_Qwag-K_Q)./K_Qwag);
eta0er= abs((eta0wag-eta0)./eta0wag);
